...
###########################################################################
## sdg via random mini batches


def fit_rmsgd_early_stopping(datasets, outpath, n_batches,
                            models, classifier, n_epochs=1000,
                            patience=5000, patience_increase=2, improvement_threshold=0.995):
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_tn_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                validation_losses = [v_model() for i in xrange(n_v_batches)]
                this_validation_loss = np.mean(validation_losses)
                print(
                    'epoch %i, minibatch %i/%i, validation error %f, minibatch average cost %f' %
                    (
...
class RandomMiniBatchSGD(object):
    def __init__(self, index, x, y, batch_size, learning_rate,
                 datasets, outpath, learner, cost, updates=None):
    ...
    def _random_indices(self):
        n_tn = tn.get_value(borrow=True).shape[0]
        n_v = v.get_value(borrow=True).shape[0]
        n_tt = tt.get_value(borrow=True).shape[0]

        tn_indices = self.rng.choice(size=(self.batch_size,), a=n_tn)
        v_indices = self.rng.choice(size=(self.batch_size,), a=n_v)
        tt_indices = self.rng.choice(size=(self.batch_size,), a=n_tt)
        return [tn_indices, v_indices, tt_indices]
    ...
    def fit(self, patience=5000, n_epochs=1000,
        ...
        return fit_rmsgd_early_stopping(
            self.datasets,
            self.outpath,
            self.n_batches,
            self.models,
...
class SupervisedRandomMSGD(RandomMiniBatchSGD):
    def __init__(self, index, x, y, batch_size, learning_rate,
                 datasets, outpath, learner, cost):
    ...
    def _compile_models(self):
        ...
        tn_model = theano.function(
            ...
            updates=self.updates,
            givens={
                self.x: tn_x[self.tn_indices],
                self.y: tn_y[self.tn_indices]
            }
        )
        v_model = theano.function(
            inputs=[],
            outputs=self.learner.errors(self.y),
            givens={
                self.x: v_x[self.v_indices],
                self.y: v_y[self.v_indices]
            }
        )
        tt_model = theano.function(
            inputs=[self.index],
            outputs=self.learner.errors(self.y),
            givens={
                self.x: tt_x[self.tt_indices],
                self.y: tt_y[self.tt_indices]
            }
        )
        return [tn_model, v_model, tt_model]
